...
from src.models.diffusion import VPDiffusionSchedule, run_sampler, EDMDiffusionSchedule
...
class EpicDiffusionGenerator(pl.LightningModule):
    ...
    def __init__(
        ...
        *,
        ...
        self.diff_sched0 = EDMDiffusionSchedule(**diff_config0)
    ...
    def get_scalings(self, sigma: T.Tensor) -> Tuple[T.Tensor, T.Tensor, T.Tensor]:
        c_skip = self.sigma_data ** 2 / (sigma ** 2 + self.sigma_data ** 2)
        c_out = sigma * self.sigma_data / (sigma ** 2 + self.sigma_data ** 2) ** 0.5
        c_in = 1 / (sigma ** 2 + self.sigma_data ** 2) ** 0.5
        return c_skip, c_out, c_in
    
    def weighting_soft_min_snr(self, sigma: T.Tensor) -> T.Tensor:
    ...
    def _shared_step(self, sample: tuple) -> Tuple[T.Tensor, T.Tensor]:
        ...
        sigma = (diffusion_times * 1.2 -1.2).exp()

        # Mix the signal and noise according to the diffusion equation
        #noisy_nodes = signal_rates * nodes + noise_rates * noises
        noisy_nodes = nodes + sigma.view(-1, 1, 1) * noises

        c_skip, c_out, c_in = self.get_scalings(sigma.view(-1, 1, 1))
        c_weight = self.weighting_soft_min_snr(sigma.view(-1, 1, 1))

        # Predict the noise using the network
        pred_noises = self.forward(noisy_nodes*c_in, sigma, mask, ctxt)
        ...
        simple_loss = (((pred_noises[mask] - target[mask]) ** 2).flatten(1).mean(1) * c_weight).sum()
        #print(pred_noises.shape)
        #print(c_weight.shape)
        #simple_loss = self.loss_fn(pred_noises[mask], target[mask])
