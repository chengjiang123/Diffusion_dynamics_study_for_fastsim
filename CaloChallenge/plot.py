...
parser.add_argument('--data_folder', default='../../CaloDiffusion', help='Folder containing data and MC files')
parser.add_argument('--plot_folder', default='../plot1', help='Folder to save results')
parser.add_argument('--generated', '-g', default='', help='Generated showers')
parser.add_argument('--model_loc', default='../', help='Location of model')
...
if(not flags.sample):
    ...
    def HistERatio(data_dict,true_energies):
        ...
        for plt_ext in plt_exts: fig.savefig('{}/FCC_ERatio_{}_{}_{}_{}.{}'.format(flags.plot_folder,dataset_config['CHECKPOINT_NAME'],flags.model,flags.sample_algo,flags.sample_steps, plt_ext))
        return feed_dict
    ...
    def ScatterESplit(data_dict,true_energies):
        ...
        for plt_ext in plt_exts: fig.savefig('{}/FCC_Scatter_{}_{}_{}_{}.{}'.format(flags.plot_folder,dataset_config['CHECKPOINT_NAME'],flags.model,flags.sample_algo,flags.sample_steps, plt_ext))
    ...
    def AverageER(data_dict):
        ...
        for plt_ext in plt_exts: fig.savefig('{}/FCC_Energy{}_{}_{}_{}_{}.{}'.format(flags.plot_folder,f_str, dataset_config['CHECKPOINT_NAME'],flags.model,flags.sample_algo,flags.sample_steps, plt_ext))
        return feed_dict
    ...
    def HistEtot(data_dict):
        ...
        ax0.set_xscale("log")
        for plt_ext in plt_exts: fig.savefig('{}/FCC_TotalE_{}_{}_{}_{}.{}'.format(flags.plot_folder,dataset_config['CHECKPOINT_NAME'],flags.model,flags.sample_algo,flags.sample_steps, plt_ext))
        return feed_dict
    ...
    def HistVoxelE(data_dict):
        ...
        ax0.set_xscale("log")
        for plt_ext in plt_exts: fig.savefig('{}/FCC_VoxelE_{}_{}_{}_{}.{}'.format(flags.plot_folder,dataset_config['CHECKPOINT_NAME'],flags.model,flags.sample_algo,flags.sample_steps, plt_ext))
        return feed_dict
    ...
    def Plot_Shower_2D0(data_dict):
        ...
        for layer in layer_number:
            ...
            num_splits = 400
            radii = np.array([0.0, 4.65, 9.3, 13.95, 18.6, 23.25, 27.9, 32.55, 37.2, 41.85])
            theta, rad = np.meshgrid((2.*np.pi*np.arange(num_splits+1)/ num_splits)[:-1], radii[:-1])
            ...
            for ik,key in enumerate(['Geant4',utils.name_translate[flags.model]]):
                ...
                pts_per_angular_bin = int(num_splits / 16)
                data_repeated = np.repeat(average[:,:,0], (pts_per_angular_bin), axis=0)
                data_repeated = np.transpose(data_repeated)
                # Set the facecolor of the polar axes to 'none'
                ax.set_facecolor('none')

                # Set the background color of the figure to be transparent
                fig.patch.set_alpha(0)
                
                im = ax.pcolormesh(theta, rad, data_repeated, cmap=cmap)
                ...
                fig.savefig('{}/FCC_{}2D_{}_{}_{}_{}_{}.pdf'.format(flags.plot_folder,key,layer,dataset_config['CHECKPOINT_NAME'],flags.model,flags.sample_algo,flags.sample_steps, plt_exts), transparent=True)
    ...
    #if(do_cart_plots):
    plot_routines['2D average shower']=Plot_Shower_2D0
