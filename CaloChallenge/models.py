...
#Unet with conditional layers
    def __init__(
        ...
        #else: cond_layers = [nn.Unflatten(-1, (-1, 2)), nn.Linear(2, half_cond_dim//2),nn.GELU()]
        cond_layers += [ nn.Linear(half_cond_dim//2, half_cond_dim), nn.GELU(), nn.Linear(half_cond_dim, half_cond_dim)]
    ...
    def forward(self, x, cond, time):
        ...
        t = self.time_mlp(time)
        #cond1 = torch.stack([cond.clone(),cond],dim=1)
        #print('cond{}'.format(cond.shape))
        c = self.cond_mlp(cond)
        #print('c{}'.format(c.squeeze(1).shape))
        #print('t{}'.format(t.shape))
        conditions = torch.cat([t,c], axis = -1)
        #conditions = torch.cat([t,c.squeeze(1)], axis = -1)
        #print('condition{}'.format(conditions.shape))
