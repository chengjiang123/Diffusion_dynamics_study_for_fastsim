...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, (E,data) in tqdm(enumerate(loader_train, 0), unit="batch", total=len(loader_train)):
            ...
            rnd_norm = torch.randn((data.size()[0],), device=data.device)

            sigma = (rnd_norm * 1.2 - 1.2).exp()
            ...
            batch_loss = model.compute_loss(data, E, noise = noise, t = sigma, loss_type = loss_type, energy_loss_scale = energy_loss_scale)
            batch_loss.backward()
        ...
        for i, (vE, vdata) in tqdm(enumerate(loader_val, 0), unit="batch", total=len(loader_val)):
            ...
            rnd_norm = torch.randn((vdata.size()[0],), device=vdata.device)

            sigma = (rnd_norm * 1.2 - 1.2).exp()
            noise = torch.randn_like(vdata)
            if(cold_diffu): noise = model.gen_cold_image(vE, cold_noise_scale, noise)

            batch_loss = model.compute_loss(vdata, vE, noise = noise, t = sigma, loss_type = loss_type, energy_loss_scale = energy_loss_scale)
