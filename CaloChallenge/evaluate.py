...
#parser.add_argument('--input_file1', '-i1', help='Name of the input file1 to be evaluated.')
...
parser.add_argument('--niter', '-n', default = 0, help='number of times')
...
class DNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, num_hidden, input_dim, dropout_probability=0.):
        ...
        #self.outputlayer = torch.nn.Linear(num_hidden, 3)
...
def prepare_low_data_for_classifier(hdf5_file, hlf_class, label, normed=False):
    ...
    if normed:
        ...
        ret = np.concatenate([np.log10(E_inc[:,label]), voxel[:,:,label], np.log10(E_norm[:,label]+1e-8),
                                      label*np.ones_like(E_inc[:,label])], axis=1)
    else:
        voxel = voxel / E_inc
...
def ttv_split(data1, data2, split=np.array([0.55, 0.3, 0.15])):
...
def train_cls(model, data_train, optim, epoch, arg):
    ...
    for i, data_batch in enumerate(data_train):
        ...
        #_, pred = torch.max(torch.softmax(output_vector.detach(), dim=1), 1)
        
        #log_posterior = torch.log_softmax(output_vector.detach(), dim=1)
        ...
        if i == 0:
            res_true = target
            res_pred = pred
            #res_log_posterior = log_posterior
        else:
            res_true = torch.cat((res_true, target), 0)
            res_pred = torch.cat((res_pred, pred), 0)
            #res_log_posterior = torch.cat((res_log_posterior, log_posterior), 0)

    try:
        print("Accuracy on training set is",
          accuracy_score(res_true.cpu(), np.clip(res_pred.cpu(), 0., 1.0)))
        #print("log poster is {}".format(res_log_posterior))
    except:
        print("Nans")
...
def evaluate_cls(model, data_test, arg, final_eval=False, calibration_data=None):
    ...
    for j, data_batch in enumerate(data_test):
        ...
        result_true_list.append(target.cpu().numpy())
        result_pred_list.append(pred.cpu().numpy())
    BCE = torch.nn.BCEWithLogitsLoss()(result_pred, result_true)
    
    # Convert lists to numpy arrays
    result_true_np = np.concatenate(result_true_list)
    result_pred_np = np.concatenate(result_pred_list)
    ...
    if final_eval:
        ...
        log_posterior_probs = np.log(torch.sigmoid(torch.from_numpy(result_pred_np)).numpy())
        mean_log_posterior = np.mean(log_posterior_probs)

        # Print the result
        print("There are {} Log Posterior Probabilities".format(log_posterior_probs.shape))
        print("Log Posterior Probabilities:\n", log_posterior_probs)
        print("Mean Log Posterior Probabilities:\n", mean_log_posterior)
...
if __name__ == '__main__':
    ...
    #source_file1 = h5py.File(args.input_file1, 'r')
    #check_file(source_file1, args, which='input1')
    ...
    #shower1, energy1 = extract_shower_and_energy(source_file1, which='input1')
    ...
    if args.mode in ['all', 'cls-low', 'cls-high', 'cls-low-normed']:
        ...
        if args.mode in ['cls-low']:
            print("Doing low level classification")
            source_array = prepare_low_data_for_classifier(source_file, hlf, 0.,
                                                           normed=False)
            
            #source_array1 = prepare_low_data_for_classifier(source_file, hlf, 1.,
                                                           #normed=False)
            
            #source_array_all = np.concatenate((source_array,source_array1),axis=0)
            
            reference_array = prepare_low_data_for_classifier(reference_file, reference_hlf, 2.,
                                                              normed=False)
        elif args.mode in ['cls-low-normed']:
            print("Doing norm low level classification")
            source_array = prepare_low_data_for_classifier(source_file, hlf, 0.,
                                                           normed=True)
            reference_array = prepare_low_data_for_classifier(reference_file, reference_hlf, 1.,
                                                              normed=True)
        elif args.mode in ['all', 'cls-high']:
            print("Doing high level classification")
    ...
    if args.mode in ['fpd', 'kpd']:
        ...
        print(result_str)
        with open(os.path.join(args.output_dir, 'fpd_kpd_{}.txt'.format(args.dataset)), 'w', encoding='utf-8') as f:
            f.write(result_str)
