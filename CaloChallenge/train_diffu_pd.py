...
if __name__ == '__main__':
    ...
    diffu_checkpoint_path = os.path.join("../trained_models/dataset2.pth")
    diffu_checkpoint = torch.load(diffu_checkpoint_path, map_location = device)


    shape = dataset_config['SHAPE_PAD'][1:] if (not orig_shape) else dataset_config['SHAPE_ORIG'][1:]
    
    model = CaloDiffu_pd(shape, config=dataset_config, training_obj = training_obj, NN_embed = NN_embed, nsteps = dataset_config['NSTEPS'],
                cold_diffu = cold_diffu, avg_showers = avg_showers, std_showers = std_showers, E_bins = E_bins ).to(device = device)
    
    ema_model = CaloDiffu_pd(shape, config=dataset_config, training_obj = training_obj, NN_embed = NN_embed, nsteps = dataset_config['NSTEPS'],
                cold_diffu = cold_diffu, avg_showers = avg_showers, std_showers = std_showers, E_bins = E_bins ).to(device = device)
    
    model0 = CaloDiffu(shape, config=dataset_config, training_obj = training_obj, NN_embed = NN_embed, nsteps = dataset_config['NSTEPS'],diffu_model=model,
                cold_diffu = cold_diffu, avg_showers = avg_showers, std_showers = std_showers, E_bins = E_bins ).to(device = device)
        #sometimes save only weights, sometimes save other info
    #if('model_state_dict' in diffu_checkpoint.keys()): model0.load_state_dict(diffu_checkpoint['model_state_dict'])
    #elif(len(diffu_checkpoint.keys()) > 1): model0.load_state_dict(diffu_checkpoint)
    ...
    new_state_dict = {}
    for key,value in diffu_checkpoint['model_state_dict'].items():
        new_key = key.replace("model.","CaloDiffuModel.model.")
        new_state_dict[new_key] = value

    else: #init from diffusion model
        model.load_state_dict(diffu_checkpoint['model_state_dict'])
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, (E,data) in tqdm(enumerate(loader_train, 0), unit="batch", total=len(loader_train)):
            ...
            #print("nsteps{}".format(model.nsteps))

            t = 2*torch.randint(0, model.nsteps, (data.size()[0],), device=device).long()
            #print(max(t))
            ...
            batch_loss = model0.compute_loss(data, E, noise = noise, t = t, loss_type = 'pd', energy_loss_scale = energy_loss_scale)
            #torch.cuda.memory_summary(device=None, abbreviated=False)
            batch_loss.backward()

            optimizer.step()
            moving_average(model, ema_model)
        ...
        for i, (vE, vdata) in tqdm(enumerate(loader_val, 0), unit="batch", total=len(loader_val)):
            ...
            t = 2*torch.randint(0, model.nsteps, (vdata.size()[0],), device=device).long()
            noise = torch.randn_like(vdata)
            if(cold_diffu): noise = model.gen_cold_image(vE, cold_noise_scale, noise)

            batch_loss = model0.compute_loss(vdata, vE, noise = noise, t = t, loss_type = 'pd', energy_loss_scale = energy_loss_scale)
